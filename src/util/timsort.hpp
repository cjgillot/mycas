...
//             LOG("initRunLen: " << initRunLen);
    ...
    static diff_t
    countRunAndMakeAscending(const iter_t &lo, const iter_t &hi, compare_t compare) {
        assert( lo < hi );
...
//             LOG("ascending");
    ...
    void mergeLo(iter_t const &base1, diff_t len1, const iter_t &base2, diff_t len2) {
    ...
    friend void timsort(const IterT &first, const IterT &last, CompT c);
};

template<typename Iter, typename Less>
inline void timsort(const Iter &first, const Iter &last, Less c) {
    TimSort<Iter, Less>::sort(first, last, c);
}

} // namespace detail

using detail::timsort;
