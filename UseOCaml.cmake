# source : https://github.com/magthe/cmake-ocaml.git
...
macro( ocaml_set_target_deplibs target )
    ...
    foreach( lib ${OCAML_${target}_LIBRARIES} )
        list( APPEND OCAML_${target}_TARGET_DEPLIBS ${lib} )
    endforeach()
endmacro()
...
macro( ocaml_set_target_translibs target )
    ...
    foreach( lib ${OCAML_${target}_LIBRARIES} )
        get_target_property( translibs ${lib} TARGET_TRANSLIBS )
...
macro( ocaml_set_target_transpkgs target )
    ...
    foreach( lib ${OCAML_${target}_LIBRARIES} )
        get_target_property( transpkgs ${lib} TARGET_TRANSPKGS )
...
macro( ocaml_get_srcs_lex target srcfile srclist )
    ...
    file( MAKE_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir )
    execute_process(
        COMMAND ocamllex ${CMAKE_CURRENT_SOURCE_DIR}/${srcfile}
            -o ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${srcfile_we}.ml
        OUTPUT_QUIET
        )
    list( APPEND ${srclist} ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${srcfile_we}.ml )
endmacro()
...
macro( ocaml_get_srcs_yacc target srcfile srclist )
    ...
    execute_process(
        ...
        COMMAND mv
            ...
            ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${srcfile_we}.mli
        COMMAND mv
            ${CMAKE_CURRENT_SOURCE_DIR}/${srcfile_we}.ml
            ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${srcfile_we}.ml
        OUTPUT_QUIET
        )
    list( APPEND ${srclist}
        ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${srcfile_we}.mli
...
macro( ocaml_set_real_srcs target )
    ...
    foreach( src ${OCAML_${target}_SOURCES} )
        ...
        elseif( ext STREQUAL ".mly" ) # entry is a yacc file
            ...
            if( EXISTS "${src}.ml" )
                list( APPEND srcfiles "${src}.ml")
            endif()
            if( EXISTS "${src}.mli" )
                list( APPEND srcfiles "${src}.mli")
            endif()
            if( EXISTS "${src}.mll" )
...
macro( ocaml_get_dependencies target srcfile dependencies )
    ...
    foreach( dep_mod ${dep_mods} )
        ...
        foreach( tgtsrc ${OCAML_${target}_SOURCES} )
            ...
            if( dep_fn STREQUAL ${tgtsrcname} )
                ...
                set( location "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir" )
    ...
    if( is_impl AND has_intf )
        list( APPEND ${dependencies} "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${srcname_we}.cmi" )
    endif()
...
macro( ocaml_add_impl_obj target srcfile )
    ...
    set( cmo_name "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${name_we}.cmo" )
    set( cmx_name "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${name_we}.cmx" )
    set( obj_name "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${name_we}.o" )
    set( annot_name "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${name_we}.annot" )
    set( cmi_name "" )
    ...
    if( NOT EXISTS "${path}/${name_we}.mli" )
        set( cmi_name "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${name_we}.cmi" )
...
macro( ocaml_add_intf_obj target srcfile )
    ...
    set( output "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${name_we}.cmi" )
    #message( STATUS "   ${srcfile} output: ${output}" )
...
macro( ocaml_add_archives target )
    ...
    # {{{2 cma archive
    set( cmaoutput "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${target}.cma" )
    ...
    # {{{2 cmx archive
    set( cmxoutput "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${target}.cmxa" )
    set( liboutput "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${target}.a" )
    ...
    set( OCAML_${target}_ARCHIVES ${target}.cma ${target}.cmxa )
endmacro()
...
macro( ocaml_install_library pkg lib )
    ...
    get_target_property( obj_target ${lib} OBJ_TARGET )
    #message( STATUS "       obj_target: ${obj_target}" )
    get_target_property( archives ${lib} ARCHIVES )
...
# {{{1 ocaml_add_exe
macro( ocaml_add_output_obj target )
    ...
    # {{{2 bytecode obj
    set( objoutput "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${target}.o" )
    add_custom_command( OUTPUT ${objoutput}
        ...
        WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
        COMMENT "Building OCaml output object ${target}.o"
        )
    add_custom_target( ${target}.o DEPENDS ${objoutput} )
    if( cmatargets )
        add_dependencies( ${target}.o ${cmatargets} )
    endif()
    ...
    # {{{2 optimised obj
    set( optoutput "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${target}.opt.o" )
    add_custom_command( OUTPUT ${optoutput}
        ...
        WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
        COMMENT "Building OCaml output object ${target}.opt.o"
        )
    add_custom_target( ${target}.opt.o DEPENDS ${optoutput} )
    if( cmxatargets )
        add_dependencies( ${target}.opt.o ${cmxatargets} )
    endif()
    set_target_properties( ${target}.opt.o PROPERTIES
        OUTPUT ${optoutput}
        )

    # {{{2 set target's OBJ
    set( OCAML_${target}_OBJ ${target}.o ${target}.opt.o )
endmacro()

# {{{1 add_ocaml_executable
macro( add_ocaml_output_obj target )
    ...
    ocaml_add_output_obj( ${target} )
    #message( STATUS "   OCAML_${target}_OBJ:              ${OCAML_${target}_OBJ}" )
    ...
    set_target_properties( ${target} PROPERTIES
        ...
        REAL_SRCS "${OCAML_${target}_REAL_SRCS}"
        OBJ_TARGET "${OCAML_${target}_OBJ_TARGETS}"
        OUTPUT_OBJ "${OCAML_${target}_OBJ}"
        )
    add_dependencies( ${target} ${OCAML_${target}_OBJ_TARGETS} ${OCAML_${target}_OBJ} )
endmacro()
...
macro( ocaml_add_exe target )
    ...
    # {{{2 bytecode exe
    set( exeoutput "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${target}" )
    ...
    # {{{2 optimised exe
    set( optoutput "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${target}_opt" )
    if( OCAML_${target}_RUN_POST_BUILD )
        add_custom_command( OUTPUT ${optoutput}
            COMMAND ${CMAKE_OCAML_COMPILER_OPT}
                -o ${optoutput}
...
macro( add_ocaml_executable target )
    ...
    add_dependencies( ${target} ${OCAML_${target}_OBJ_TARGETS} ${OCAML_${target}_EXE} )
endmacro()
...
macro( ocaml_add_c_obj target srcfile )
    ...
    set( output ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/${name_we}.o )
    add_custom_command( OUTPUT ${output}
        COMMAND mkdir -p ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir
...
macro( add_ocaml_c_library target )
    ...
    #set( output_a ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/lib${target}.a )
    #set( output_dll ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir/dll${target}.so )
    #set( output "${output_a};${output_dll}" )
    #add_custom_command( OUTPUT ${output}
    #    COMMAND ocamlmklib -oc ${target}
    #        ${OCAML_${target}_OBJS}
    #    DEPENDS ${OCAML_${target}_OBJS}
    #    WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${target}.dir
...
macro( install_ocaml_executable exe )
    ...
    get_target_property( tgts ${exe} EXE )
